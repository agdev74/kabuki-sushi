...
const cartTranslations = {
  fr: {
    titleCart: "Mon Panier", titleCheckout: "Validation", titlePayment: "Paiement Sécurisé", emptyCart: "Votre panier est vide", items: "article", itemsPlural: "articles", clearCart: "Vider le panier", name: "Nom Complet *", namePlaceholder: "Jean Dupont", phone: "Téléphone *", date: "Date de retrait/livraison *", time: "Heure souhaitée *", pickupMode: "Mode de retrait *", takeaway: "À Emporter", delivery: "Livraison", address: "Adresse *", addressPlaceholder: "Rue des Alpes 12", zip: "NPA *", floor: "Étage", floorPlaceholder: "Ex: 4", code: "Code", codePlaceholder: "Ex: A123", comments: "Instructions / Allergies", commentsPlaceholder: "Sans wasabi...", totalEstimated: "Total à payer", btnValidate: "Passer à la caisse", btnPay: "Payer la commande", minOrderError: "Minimum 25 CHF requis.", noTimeSlots: "Aucun horaire disponible.", today: "Aujourd'hui", tomorrow: "Demain", sending: "Génération du paiement...", processing: "Paiement en cours...", paymentError: "Le paiement a échoué.", successTitle: "Paiement réussi !", successDesc: "Votre commande a été validée.", btnClose: "Fermer", cancelPayment: "Annuler et modifier"
  },
  en: {
    titleCart: "My Cart", titleCheckout: "Checkout", titlePayment: "Secure Payment", emptyCart: "Empty cart", items: "item", itemsPlural: "items", clearCart: "Clear", name: "Full Name *", namePlaceholder: "John Doe", phone: "Phone *", date: "Date *", time: "Time *", pickupMode: "Method *", takeaway: "Takeaway", delivery: "Delivery", address: "Address *", addressPlaceholder: "Street", zip: "ZIP *", floor: "Floor", floorPlaceholder: "Ex: 4", code: "Code", codePlaceholder: "Ex: A123", comments: "Instructions", commentsPlaceholder: "Allergies...", totalEstimated: "Total", btnValidate: "Checkout", btnPay: "Pay Now", minOrderError: "Min 25 CHF required.", noTimeSlots: "No slots.", today: "Today", tomorrow: "Tomorrow", sending: "Preparing...", processing: "Processing...", paymentError: "Payment failed.", successTitle: "Success!", successDesc: "Order confirmed.", btnClose: "Close", cancelPayment: "Cancel"
  },
  es: {
...
function StripeCheckoutForm({ total, onSuccess, onCancel, t }: StripeCheckoutFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // ✅ FIX VERCEL : On récupère l'objet complet "result" pour éviter l'erreur de type
    ...
    <form onSubmit={handleSubmit} className="flex flex-col h-full">
      ...
      <div className="p-6 border-t border-neutral-800 bg-neutral-900 space-y-3">
        <button 
          ...
          className={`w-full font-bold py-4 rounded-xl uppercase flex items-center justify-center gap-2 ${
            isProcessing ? "bg-neutral-800 text-neutral-500" : "bg-green-600 text-white shadow-lg shadow-green-900/20"
          }`}
        >
          {isProcessing ? <><Loader2 size={18} className="animate-spin" /> {t.processing}</> : `${t.btnPay} (${total.toFixed(2)} CHF)`}
        </button>
        <button type="button" onClick={onCancel} className="w-full text-gray-500 text-[10px] font-bold uppercase py-2 hover:text-white transition">
          {t.cancelPayment}
        </button>
      </div>
    </form>
...
export default function CartDrawer({ isOpen, onClose }: CartDrawerProps) {
  ...
  const [selectedDate, setSelectedDate] = useState<Date | null>(availableDays[0]);
  ...
  const handlePaymentSuccess = () => { clearCart(); setIsPayment(false); setIsSuccess(true); };
          ...
          <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} className="fixed top-0 right-0 h-full w-full md:w-[450px] bg-neutral-900 border-l border-neutral-800 z-[101] flex flex-col">
            ...
            {isSuccess ? (
              <div className="flex-1 flex flex-col items-center justify-center p-8 space-y-6 text-center">
                ...
                <h2 className="text-2xl font-bold text-white uppercase tracking-widest">{t.successTitle}</h2>
                ...
                <button onClick={onClose} className="w-full bg-neutral-800 text-white font-bold py-4 rounded-xl uppercase hover:bg-neutral-700 transition">{t.btnClose}</button>
              </div>
              ...
              <Elements options={{ clientSecret, appearance: { theme: 'night', variables: { colorPrimary: '#dc2626' } } }} stripe={stripePromise}>
                <StripeCheckoutForm total={totalPrice} onSuccess={handlePaymentSuccess} onCancel={() => setIsPayment(false)} t={t} />
              </Elements>
            ) : (
              <div className="flex-1 flex flex-col overflow-hidden">
                  ...
                  {items.length === 0 ? <p className="text-center text-gray-500 uppercase tracking-widest py-20">{t.emptyCart}</p> : 
                    !isCheckout ? (
                      <div className="space-y-6">
                        <div className="text-[10px] text-gray-500 font-bold uppercase tracking-widest border-b border-neutral-800 pb-2">{totalItems} {totalItems > 1 ? t.itemsPlural : t.items}</div>
                        {items.map(i => (
                      ...
                      <form id="checkout-form" onSubmit={handlePreparePayment} className="space-y-6">
                        ...
                        <div className="space-y-2">
                          <label className="text-[10px] font-bold text-kabuki-red uppercase tracking-widest flex items-center gap-2"><Calendar size={12} /> {t.date}</label>
                          <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                            {availableDays.map((d, idx) => (
                              <button key={idx} type="button" onClick={() => { setSelectedDate(d); setSelectedTime(""); }} className={`shrink-0 px-4 py-2 rounded-xl border text-xs font-bold transition ${selectedDate?.toDateString() === d.toDateString() ? "bg-kabuki-red border-kabuki-red text-white" : "bg-neutral-800 border-neutral-700 text-gray-400 hover:text-white"}`}>
                                {d.toDateString() === new Date().toDateString() ? t.today : d.toDateString() === new Date(Date.now() + 86400000).toDateString() ? t.tomorrow : d.toLocaleDateString(lang, { day: 'numeric', month: 'short' })}
                              </button>
                            ))}
                          </div>
                        </div>
                        <div className="space-y-2">
                          <label className="text-[10px] font-bold text-kabuki-red uppercase tracking-widest flex items-center gap-2"><Clock size={12} /> {t.time}</label>
                          <div className="grid grid-cols-4 gap-2">
                            {availableSlots.map(s => <button key={s} type="button" onClick={() => setSelectedTime(s)} className={`py-2 rounded-lg border text-xs font-bold transition ${selectedTime === s ? "bg-kabuki-red border-kabuki-red text-white" : "bg-neutral-800 border-neutral-700 text-gray-400"}`}>{s}</button>)}
                          </div>
                        </div>
                        <div className="space-y-1">
                        ...
                        <div className="space-y-1 pt-4 border-t border-neutral-800">
                          <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2"><MessageSquare size={12} /> {t.comments}</label>
                          <textarea value={formData.comments} onChange={e => setFormData({...formData, comments: e.target.value})} className="w-full bg-black text-white border border-neutral-800 rounded-xl px-4 py-3 outline-none focus:border-kabuki-red transition h-20 resize-none" placeholder={t.commentsPlaceholder} />
                        </div>
                      </form>
                    )
                  }
                ...
                {items.length > 0 && (
                  <div className="p-6 border-t border-neutral-800 bg-neutral-900 shrink-0">
                    <div className="flex justify-between items-center mb-4"><span className="text-gray-400 text-xs font-bold uppercase tracking-widest">{t.totalEstimated}</span><span className="text-2xl font-display font-bold text-white">{totalPrice.toFixed(2)} <span className="text-kabuki-red text-sm">CHF</span></span></div>
                    {!isCheckout ? <button onClick={() => setIsCheckout(true)} className="w-full bg-kabuki-red text-white font-bold py-4 rounded-xl uppercase flex items-center justify-center gap-2 hover:bg-red-700 transition">{t.btnValidate} <ArrowRight size={16} /></button> : <button type="submit" form="checkout-form" disabled={!isFormReady || isSubmitting} className={`w-full font-bold py-4 rounded-xl uppercase flex items-center justify-center gap-2 transition ${isFormReady && !isSubmitting ? "bg-kabuki-red text-white hover:bg-red-700" : "bg-neutral-800 text-neutral-500 cursor-not-allowed"}`}>{isSubmitting ? <><Loader2 size={18} className="animate-spin" /> {t.sending}</> : <><ShieldCheck size={18} /> Continuer vers le paiement</>}</button>}
                  </div>
                )}
              </div>
            )}
